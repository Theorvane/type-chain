...
> **Release status:** This guide documents the published `@theorvane/type-chain@0.2.3` package. Every optional integration stays behind a dedicated subpath, so importing the root metadata package does not load LangChain or TypeMCP peers.
...
```bash
npm install @theorvane/type-chain@0.2.3
```
